...
export function evaluateConversion(
  ...
  if (!Number.isFinite(value)) return invalid('Please enter a number.')

  if (categoryId === 'temperature') {
    const minimum = ABSOLUTE_ZERO[fromUnitId]
    if (minimum !== undefined && value < minimum) {
      return invalid(`Nothing can be colder than absolute zero (${minimum}°).`)
    }
  } else if (value < 0) {
    return invalid('This measurement cannot be negative.')
  }

  return ok(convert(categoryId, fromUnitId, toUnitId, value))
}
